...
def _find_adb_directory() -> str:
  ...
  potential_paths = [
      ...
      '/data/xgx/android_world/Android/Sdk/platform-tools/adb',
  ]
  for path in potential_paths:
    if os.path.isfile(path):
      return path
  return "adb"  # Fallback to system adb if not found
...
_TASK_NAME = flags.DEFINE_string(
    'task_name',
    None,
    'Optional override for the task name recorded in trace as task_id.'
    ' Uses task registry name if not set.',
)
...
def _get_agent(
  ...
  # OpenRouter.
  elif _AGENT_NAME.value == 't3a_openrouter':
    agent = t3a.T3A(env, infer.OpenRouterWrapper('qwen/qwen3-vl-8b-instruct'))
  elif _AGENT_NAME.value == 'm3a_openrouter':
    temp = float(os.environ.get('LLM_TEMPERATURE', '0.0'))
    model = os.environ.get('OPENROUTER_MODEL_NAME', 'qwen/qwen3-vl-8b-instruct')
    agent = m3a.M3A(env, infer.OpenRouterWrapper(model, temperature=temp))
  elif _AGENT_NAME.value == 'm3a_local':
    # 从环境变量读取 TEMPERATURE，如果没设置则默认为 0.0（严谨模式）
    temp = float(os.environ.get('LLM_TEMPERATURE', '0.0'))
    model = os.environ.get('VLLM_MODEL_NAME', 'qwen3-vl-8b')
    # 增加重试次数到 5 次，应对 4 路并行时的偶发超时
    agent = m3a.M3A(env, infer.VllmWrapper(model, temperature=temp, max_retry=5))
  # SeeAct.
...
def _main() -> None:
  ...
  if os.environ.get('ANDROID_WORLD_RUNNING') == '1':
      ...
      return
  os.environ['ANDROID_WORLD_RUNNING'] = '1'
  ...
  try:
      env = env_launcher.load_and_setup_env(
          ...
          grpc_port=_GRPC_PORT.value, # 确保传入 grpc_port
      )
      ...
      suite_utils.run(
          ...
          agent,
          demo_mode=False,
          task_name_override=_TASK_NAME.value,
      )
      print(
      ...
      if 'env' in locals():
          env.close()
      os.environ['ANDROID_WORLD_RUNNING'] = '0'
